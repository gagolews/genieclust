...
def test_genie_approx(metric='euclidean'):
    for dataset in ["t4_8k", "h2mg_64_50", "h2mg_1024_50"]:#, "bigger"]:#[, "bigger""s1", "Aggregation", "unbalance", "h2mg_64_50"]:#, "h2mg_1024_50", "t4_8k", "bigger"]:
        ...
        for g in [0.01, 0.3, 0.7]:
            ...
            t01 = time.time()
            res1 = genieclust.Genie(k, gini_threshold=g, exact=True, affinity=metric, verbose=verbose).fit_predict(X)+1
            t11 = time.time()
            ...
            t03 = time.time()
            res3 = genieclust.Genie(k, gini_threshold=g, exact=False, affinity=metric, verbose=verbose).fit_predict(X)+1
            t13 = time.time()
