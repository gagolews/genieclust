...
if cutting is not None:
    ...
    plt.text(idx, min_mst_s[cutting], cutting, ha='center', va='bottom', color=deadwood.col[mst_labels[cutting]-1])
#
...
for i in range(1, n_clusters+1):
    ...
    ax1.plot(np.arange(last, last+len_op), mst_w[op],
        color=deadwood.col[i-1])
    ax2.plot(np.arange(last, last+len_op), min_mst_s[op], c=deadwood.col[i-1], alpha=0.2)
    if cutting is not None and mst_labels[cutting] == i:
        idx = np.sum(op < cutting)
        plt.text(last+idx, min_mst_s[cutting], cutting, ha='center', va='bottom', color=deadwood.col[mst_labels[cutting]-1])
    # ce = (np.arange(1, n)*internodes)[op]   # 1-shift
    # for j in np.flatnonzero(ce):
        # idx = ce[j]-1 # unshift
        # plt.text(j, min_mst_s[idx], idx, ha='center', color=deadwood.col[mst_labels[idx]-1])
...
plt.bar(np.arange(len(s)), s[o1][o2], width=1.0, color=np.array(deadwood.col)[l[o1]-1][o2])
plt.ylim(-1, 1)
